...
def check_model_files():
    ...
    for filename, info in MODEL_URLS.items():
        if not os.path.exists(filename):
            ...
            print(f"✅ Found: {filename} ({file_size / 1024 / 1024:.1f} MB)")
    
    if not missing_files:
        print("🎉 All model files are present!")
        return True
    
    print(f"\n📥 Need to download {len(missing_files)} model file(s)")
    ...
    if success_count == len(missing_files):
        print(f"\n🎉 Successfully downloaded all {success_count} model files!")
        return True
    else:
        print(f"\n⚠️  Downloaded {success_count}/{len(missing_files)} files")
        return False

def main():
    """Main function"""
    print("🛡️  E-Raksha Model Downloader")
    print("=" * 40)
