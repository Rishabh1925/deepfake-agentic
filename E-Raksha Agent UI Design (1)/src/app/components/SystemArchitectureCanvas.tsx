...
const SystemArchitectureCanvas: React.FC<{ view?: 'overview' | 'pipeline' }> = ({ view = 'pipeline' }) => {
  ...
  const nodes: Node[] = [
    ...
    { id: 'video-input', x: 50, y: 100, label: 'Video Input', type: 'input' },
    { id: 'frame-sampler', x: 50, y: 200, label: 'Frame Sampler', type: 'input' },
    { id: 'face-detector', x: 50, y: 300, label: 'Face Detector', type: 'input' },
    { id: 'audio-extractor', x: 50, y: 400, label: 'Audio Extract', type: 'input' },

    // Layer 2: Model Bank
    { id: 'bg-model', x: 250, y: 80, label: 'BG-Model', type: 'model' },
    { id: 'av-model', x: 250, y: 160, label: 'AV-Model', type: 'model' },
    { id: 'cm-model', x: 250, y: 240, label: 'CM-Model', type: 'model' },
    { id: 'rr-model', x: 250, y: 320, label: 'RR-Model', type: 'model' },
    { id: 'll-model', x: 250, y: 400, label: 'LL-Model', type: 'model' },
    { id: 'tm-model', x: 250, y: 480, label: 'TM-Model', type: 'model' },

    // Layer 3: Agentic Intelligence
    { id: 'routing-engine', x: 450, y: 100, label: 'Routing', type: 'agent' },
    { id: 'langgraph', x: 450, y: 200, label: 'LangGraph', type: 'agent' },
    { id: 'aggregator', x: 450, y: 300, label: 'Aggregator', type: 'agent' },
    { id: 'explainer', x: 450, y: 400, label: 'Explainer', type: 'agent' },

    // Layer 4: Output
    { id: 'api-response', x: 650, y: 200, label: 'API Response', type: 'output' },
    { id: 'heatmap', x: 650, y: 300, label: 'Heatmap', type: 'output' },
  ];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      connections.forEach(conn => {
        if (state.activeModels.includes(conn.from) && Math.random() < 0.2) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      connections.forEach(conn => {
        ...
        ctx.strokeStyle = isActive ? 'rgba(59, 130, 246, 0.3)' : 'rgba(100, 116, 139, 0.2)';
      ...
      nodes.forEach(node => {
        ...
        // Label
        ctx.fillStyle = isActive ? '#FFFFFF' : 'rgba(203, 213, 225, 0.8)';
